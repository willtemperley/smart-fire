...
public class FireDAO {
	...
	//		String x = getFireCountByDate(-123.65205256969075,48.31421421030944,-123.24327664326452, 48.75880075305904, new Date(), new Date());
			...
			Collection<ActiveFire> x = getRecentFires(a.getGeometry().getEnvelopeInternal());
			for (ActiveFire activeFire : x) {
				System.out.println(activeFire.getFrp());
			}
		}

	public static List<ActiveFire> getRecentFires(Envelope env) {

		String url;
		
		url = String
				.format("http://dopa-services.jrc.ec.europa.eu/services/estation/get_recent_fires?minx=%s&miny=%s&maxx=%s&maxy=%s&srid=%s",
						env.getMinX(), env.getMinY(), env.getMaxX(), env.getMaxY(), 4326);
